...
import { Check, Download, ExternalLink, FileText, LoaderCircle, Pencil, Plus, Trash2, UploadCloud, X } from "lucide-react";
  ...
  return <Modal title={resume ? "Upload fortsetzen" : "Materialien hochladen"} className="material-dialog" closeDisabled={locked} onClose={onClose}>
    <form onSubmit={async e => {
      ...
      <label className={`material-drop${dragging ? " is-dragging" : ""}${locked ? " is-disabled" : ""}`}
        ...
        <UploadCloud size={26} aria-hidden="true" /><strong>{checking ? "Dateien werden geprüft …" : resume ? "Original-PDF erneut auswählen" : files.length ? "Weitere PDFs auswählen" : "PDFs auswählen oder hier ablegen"}</strong>
      ...
      <div className="material-upload-list">{files.map(item => <div className={`material-upload-item${item.invalid || item.error ? " has-error" : ""}${item.stage === "done" ? " is-done" : ""}`} key={item.key}>
        <div className="material-upload-icon">{item.stage === "done" ? <Check size={19} /> : <FileText size={19} />}</div>
        <div className="material-upload-info"><label><span className="material-filename">{item.file.name} · {fileSize(item.file.size)}</span><input aria-label={`Titel für ${item.file.name}`} required={!item.invalid && item.stage !== "done"} maxLength={300} disabled={busy || !!item.materialId || !!item.invalid || item.stage === "done"} value={item.title} onChange={e => patch(item.key, { title: e.target.value })} /></label>
          {item.stage === "uploading" && !item.error && <progress aria-label={`Upload ${item.file.name}`} max={100} value={item.percent} />}
...
export function MaterialsPanel({ moduleId, revision, onChange }: { moduleId: string; revision: number; onChange: () => void }) {
  ...
  return <section className="panel materials-panel" aria-labelledby="materials-heading">
    <div className="materials-heading"><div><h3 id="materials-heading">Materialien <span>{materials.filter(m => m.moduleId === moduleId && m.state === "ready").length}</span></h3><p>{demo ? "Selbst erstellte Beispiel-PDFs, keine echten Hochschulunterlagen." : "Deine Originalunterlagen – auch für deinen Lernagenten."}</p></div><button className="secondary" disabled={demo} title={demo ? "Uploads sind in der öffentlichen Demo deaktiviert" : undefined} onClick={() => { setResume(undefined); setUpload(true); }}><Plus size={15} /> PDFs hochladen</button></div>
    {error && <p role="alert" className="error-box">{error} <button className="text-button" onClick={reload}>Erneut laden</button></p>}
    {materials.length > 0 && <div className="materials-filter"><label>Dokumenttyp <select aria-label="Materialien nach Dokumenttyp filtern" value={filter} onChange={e => setFilter(e.target.value)}><option value="">Alle Dokumente</option>{materialTypes.map(t => <option key={t}>{t}</option>)}</select></label></div>}
    {loading ? <p className="materials-empty">Materialien werden geladen …</p> : !shown.length ? <p className="materials-empty">{filter ? "Noch keine Dokumente dieses Typs." : "Lege hier Skripte, Übungen und Altklausuren für dieses Modul ab."}</p> : <div className="material-list">{shown.map(m => <article className="material-row" key={m.id}>
      <div className="material-file-icon"><FileText size={21} /></div><div className="material-info"><h4>{m.title}</h4><p>{m.documentType}{m.semester ? ` · ${m.semester}` : ""} · {fileSize(m.size)}{m.state === "pending" && <strong> · Upload unvollständig</strong>}</p>{m.description && <p className="material-description">{m.description}</p>}{m.relatedMaterialId && <a className="material-related" target="_blank" rel="noopener noreferrer" href={materialHref(m.relatedMaterialId)}>Zugehörig: {materials.find(r => r.id === m.relatedMaterialId)?.title || "Unterlage"}</a>}{materials.filter(r => r.relatedMaterialId === m.id && r.state === "ready").map(r => <a className="material-related" key={r.id} target="_blank" rel="noopener noreferrer" href={materialHref(r.id)}>Zugehörig: {r.title}</a>)}</div>
      <div className="material-actions">{m.state === "ready" ? <><a className="text-button" target="_blank" rel="noopener noreferrer" href={materialHref(m.id)}><ExternalLink size={14} />Öffnen</a><a className="icon-button" aria-label={`${m.title} herunterladen`} href={materialHref(m.id,null,true)}><Download size={16} /></a></> : <button className="text-button" disabled={busy} onClick={async () => { setBusy(true); setError(""); try { await request(`/${m.id}/complete`, "POST", {}); changed(); } catch (err) { setError(message(err)); } finally { setBusy(false); } }}>Upload prüfen</button>}{m.state === "pending" && <button className="text-button" disabled={busy} onClick={() => { setResume(m); setUpload(true); }}>Fortsetzen</button>}<button className="icon-button" disabled={demo} aria-label={`${m.title} bearbeiten`} onClick={() => setEditing(m)}><Pencil size={15} /></button><button className="icon-button" disabled={demo} aria-label={`${m.title} löschen`} onClick={() => { setError(""); setDeleting(m); }}><Trash2 size={15} /></button></div>
    </article>)}</div>}
    ...
    {deleting && <Modal title="Material löschen?" className="material-dialog" closeDisabled={busy} onClose={() => { if (!busy) setDeleting(null); }}><div className="material-dialog-body"><p className="form-hint">„{deleting.title}“ und die Originaldatei werden dauerhaft gelöscht. Quellenverweise an Aufgaben müssen vorher entfernt werden.</p>{error && <p className="error-box" role="alert">{error}</p>}<div className="modal-actions"><button className="secondary" disabled={busy} onClick={() => setDeleting(null)}>Abbrechen</button><button className="primary" disabled={busy} onClick={async () => { setBusy(true); setError(""); try { await request(`/${deleting.id}`, "DELETE", { version: deleting.version, confirm: true }); setDeleting(null); changed(); } catch (err) { setError(message(err)); } finally { setBusy(false); } }}>{busy ? "Löscht …" : "Endgültig löschen"}</button></div></div></Modal>}
  </section>;
}
